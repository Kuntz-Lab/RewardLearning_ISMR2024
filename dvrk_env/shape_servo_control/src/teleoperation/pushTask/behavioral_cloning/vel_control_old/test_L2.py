...
def move_to_goal_state(policy, device, sim_cache, plan_config, data_config, args):
    ...
    while (True):
        ...
        step_physics(sim_cache)
        #gym.set_actor_dof_velocity_targets(envs[0], dvrk_handles[0], [0]*10)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', default=f"/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_push_BC_corrected/weights/weights_L2_1_more_layers", type=str, help="model path")
    ...
    model.to(device)
    model.load_state_dict(torch.load(os.path.join(model_path, "epoch 500")))
    model.eval()
