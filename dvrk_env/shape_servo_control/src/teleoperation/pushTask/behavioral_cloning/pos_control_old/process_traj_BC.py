...
if __name__ == "__main__":
    ...
    data_processed_path = f"/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_push_BC_corrected/pos_control/data_processed_{suffix}"
    os.makedirs(data_processed_path, exist_ok=True)
    num_samples_per_group = 1 #Change
    NUM_GROUP = 100 # Change
    data_idx = 0
    ...
    for i in range(NUM_GROUP):
        ...
        for j in range(0, len(eef_poses)-offset, 1):
            ...
            action = torch.tensor(eef_poses[j+offset], device=device).unsqueeze(0)
            
            processed_data = {"state": pose, "action": action}
