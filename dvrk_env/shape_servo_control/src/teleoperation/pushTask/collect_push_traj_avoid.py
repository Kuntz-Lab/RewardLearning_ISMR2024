...
def avoid_box_point(eef_pose, box_pose, box_radius):
    ...
    angle = np.arcsin(box_radius/box_eef_dist) + 0.02
    rotation_matrix = np.array([[np.cos(angle), -np.sin(angle)], \
                                [np.sin(angle), np.cos(angle)]])
    avoid_point_displace = 1.1*np.matmul(rotation_matrix, box_eef_displace)
...
def get_shape_servo_plan_state(sim_cache, plan_config, robot, dc_client):
    ...
    cone_pose.p = gymapi.Vec3(*list(eef_state["pose"]["p"]))

    pose = gymapi.Transform()
    offset_from_box = 0.04 #0.06
        ...
        if reach_goal:
            ...
            if is_near_box:
                if plan_config["sample_cp"]:
                    ...
                    rand_y = box_pose.p.y+np.random.uniform(low = -offset_from_box , high = offset_from_box)
                    while((rand_x-box_pose.p.x)**2<=min_offset_from_box**2 or (rand_y-box_pose.p.y)**2<=min_offset_from_box**2) or not is_in_workspace(rand_x, rand_y):
                        # print("hi, is in workspace?: ", is_in_workspace(rand_x, rand_y))
