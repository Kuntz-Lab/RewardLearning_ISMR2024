...
'''
collecting data of point clouds of multiple box and a single cone for visualization
'''
...
def configure_isaacgym(gym, args):
    ...
    # Load box object
    init_pose = [0.0, 0.0, 0.03+THICKNESS] 
    ...
    #pose = list(np.array(init_pose) + np.random.uniform(low=[-0.1,-0.6,0], high=[0.1,-0.4,0], size=3))
    print("****** cone xy pose: ", args.cone_xy)
    pose = [args.cone_xy[0], args.cone_xy[1], init_pose[2]]
    pose[2] += CONE_Z_OFFSET
    cone_pose.p = gymapi.Vec3(*pose)
    
    cone_asset = gym.create_box(sim, *[0.04, 0.04, 0.08], asset_options) #gym.create_sphere(sim, 0.02, asset_options)
...
def record_state(sim_cache, data_config, args):
    ...
    # print("finished stepping physics")
    ...
    pcd = np.squeeze(farthest_point_sample_batched(pcd, npoint=256), axis=0)
    data_config["partial_pc"].append(pcd)
    data_config["box_poses"].append([sim_cache["box_pose"].p.x, sim_cache["box_pose"].p.y, sim_cache["box_pose"].p.z])
    ...
    if vis:
            ...
            pcd_vis.points = open3d.utility.Vector3dVector(data_config["partial_pc"])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_recording_path', default=f"/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_push/vis_reward", type=str, help="where you want to record data")
    parser.add_argument('--rand_seed', default=1357, type=int, help="random seed")
    parser.add_argument('--num_samples', default=700, type=int, help="number of samples you want to collect")
    parser.add_argument("--cone_xy", nargs="+", type=float, help="x and y coordinates of the cone")
    ...
    data_config = {"sample_count":0, "max_sample_count":num_samples, "data_recording_path": data_recording_path, "partial_pc":[], "box_poses":[], "cone_pose":None}
    data_config["cone_pose"] = [sim_cache["cone_pose"].p.x, sim_cache["cone_pose"].p.y, sim_cache["cone_pose"].p.z]
    ...
    data_idx = len(os.listdir(data_recording_path))
    if args.save_data:
        data = data_config
        with open(os.path.join(data_recording_path, f"sample_{data_idx}.pickle"), 'wb') as handle:
