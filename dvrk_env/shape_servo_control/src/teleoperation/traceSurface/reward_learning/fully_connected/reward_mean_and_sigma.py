...
if __name__ == "__main__":
    ...
    for group_idx in range(num_group):
        ...
        for sample_idx in range(num_sample_per_group):
            ...
            prev_pc_len = -1
            for frame in range(len(partial_pcs)):
                balls_xyz = balls_xyzs_list[frame]
                partial_pc = partial_pcs[frame]
                if len(partial_pc)!=0 and prev_pc_len != len(partial_pc):
                    num_samples = 1000
                    prev_pc_len = len(partial_pc)
