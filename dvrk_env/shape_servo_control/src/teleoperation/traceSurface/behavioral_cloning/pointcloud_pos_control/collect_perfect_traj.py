...
def move_near_workspace(robot, dc_client):
    ...
    while(not success):
        ...
        rand_init_pos = np.random.uniform(low=[-0.1, -0.6, 0.011], high=[0.1,-0.4,0.2], size=3)
        cartesian_pose.position.x = -rand_init_pos[0] #0 #0.1
        cartesian_pose.position.y = -rand_init_pos[1]#-(-0.5) #-(-0.48)
        cartesian_pose.position.z = rand_init_pos[2] - ROBOT_Z_OFFSET#0.1-ROBOT_Z_OFFSET
...
def config_state(sim_cache, data_config):
    ...
    # use max_frame = 250 for random demo, max_frame=150 for perfect demo
    plan_config = {"checkpoints": checkpoints, "current_cp":0, "reach_cps": reach_cps, "which_balls_reached": [0 for i in range(len(balls_xyz))],
                  "eef_states":[], "pcds":[], "balls_poses_list": [], "last_ball_poses": [], "frame":0, "max_frame":150, "num_balls_reached": 0, "stay":False, "last_ball_pose":None} #max_frame = 210
    #250
...
def move_to_goal_state(sim_cache, plan_config, data_config, robot, plan_traj, args, record_pc=True):
    ...
    while traj_index <= len(plan_traj):
        ...
        if plan_config["frame"] < plan_config["max_frame"] - 1:
            ...
            if plan_config["num_balls_reached"] == MAX_NUM_BALLS:
                if args.save_data:
...
def reset_state(sim_cache, data_config, init_robot_state):
    ...
    resample = data_config["sample_count"] >= data_config["max_sample_count"] or plan_config["num_balls_reached"] != MAX_NUM_BALLS
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_recording_path', default=f"/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_trace_curve_corrected/demos_{suffix}_straight_3D_2ball", type=str, help="where you want to record data")
