...
def sample_balls_poses(num_balls, curve_type, args, data_config, use_demo=True):
    if use_demo:
        if data_config==None:
            group = 0
        else:
            group = data_config["group_count"]
        file = os.path.join(args.demo_path, f"group {group} sample {0}.pickle")
        with open(file, 'rb') as handle:
            demo_data = pickle.load(handle)
        
        weights_list = demo_data["weights_list"]
        xy_curve_weights = demo_data["xy_curve_weights"]
        balls_xyz = demo_data["balls_xyz"]
        balls_poses = []
...
def set_balls_poses(args, data_config, use_demo=True):
    rand_num_balls = MAX_NUM_BALLS
    weights_list, xy_curve_weights, balls_xyz, balls_poses = sample_balls_poses(num_balls=rand_num_balls, curve_type=CURVE_TYPE, args=args, data_config=data_config, use_demo=use_demo)
...
def configure_isaacgym(gym, args):
    ...
    weights_list, xy_curve_weights, balls_xyz, all_balls_poses, rand_num_balls = set_balls_poses(args, data_config=None, use_demo=args.use_demo)
...
def config_state(sim_cache, data_config):
    ...
    plan_config = {"which_balls_reached": [0 for i in range(len(balls_xyz))], "no_plan": False, 
                  "eef_states":[], "pcds":[], "balls_poses_list": [], "last_ball_poses": [], "frame":0, "max_frame":800, "num_balls_reached": 0, "last_ball_pose":None} #max_frame = 210
    #300 #210 #250
...
def reset_state(sim_cache, data_config, init_robot_state, args):
    ...
    if re_sample:
        ...
        weights_list, xy_curve_weights, balls_xyz, all_balls_poses, rand_num_balls = set_balls_poses(args, data_config, use_demo=args.use_demo)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_samples', default=1, type=int, help="number of samples per group you want to collect")
    ...
    parser.add_argument('--demo_path', type=str, help="path to the saved demonstrations, we will use the ball configuration in the demos")
    parser.add_argument('--use_demo', default="True", type=str, help="whether to use ball configuration in the demos")
    ...
    args.use_demo = args.use_demo == "True"
    ...
    max_sample_count = 1 #args.num_samples
