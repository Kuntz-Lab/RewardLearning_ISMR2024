...
def append_ball_indicators(goal_indicators, balls_left):
    '''
    if goal indicator is 0, the destination the robot is going to is the checkpoint ball (ball0)
    if goal indicator is 1, the robot has already reached ball0 and the destination the robot is going to is the goal ball (ball1)
    '''
    if len(balls_left)==2:
        goal_indicators.append(0)
    elif len(balls_left)==1:
        goal_indicators.append(1)
    else:
        # we just reached the goal
        goal_indicators.append(1)

def update_balls_left(balls_left, plan_config, is_traj_end):
    reach_checkpoint = plan_config["reach_checkpoint"]
    reach_goal = plan_config["reach_checkpoint"]
    cp_idx = plan_config["cp_idx"]
    num_cp = plan_config["num_cp"]
    max_cp = plan_config["max_cp"]

    if len(balls_left)==2:
       if reach_checkpoint and num_cp==cp_idx and is_traj_end:
            print("++++++++++++++++++++ checkpoint ball is reached")
            balls_left.pop(0)
    elif len(balls_left)==1:
        if reach_goal and num_cp==max_cp and is_traj_end:
            print("++++++++++++++++++++ goal ball is reached")
            balls_left.pop(0)
    else:
        rospy.logerr('-----------------error, should not call check_ball_reached')

def traj_seg_label(reach_goal, reach_cp):
    '''
    Returns the segmentation label for a trajectory.
    Trajectories with labels that have gcd not equal to 1 can be compared
    '''
    if not (reach_goal) and not (reach_cp):
        return 2
    elif not (reach_goal) and reach_cp:
        return 4
    elif reach_goal and reach_cp:
        return 6
    else:
        #not reach_cp and reach_goal
        return 3

def move_to_goal_state(sim_cache, plan_config, data_config, robot, plan_traj, eef_states, goal_indicators, balls_left, args):
    envs = sim_cache["envs"]
    gym = sim_cache["gym"]
    dvrk_handles = sim_cache["dvrk_handles"]
    soft_pose = sim_cache["soft_pose"]
    ...
    while traj_index < len(plan_traj):
        ...
        if frame_count % 5 == 0:
            ...
            append_ball_indicators(goal_indicators, balls_left)
        ...
        update_balls_left(balls_left, plan_config, is_traj_end=traj_index==len(plan_traj))
        ...
        if traj_index == len(plan_traj):
            ...
            assert(len(goal_indicators)==len(eef_states))          
            ...
            if plan_config["num_cp"] <=  plan_config["max_cp"] -1:
                ...
                if args.save_data:
                    ...
                    seg_label = traj_seg_label(success_goal, plan_config["reach_checkpoint"])
                    ...
                    data = {"traj": eef_states,\
                            ...
                            "num cp": plan_config["max_cp"], "goal indicators": goal_indicators, "traj seg label": seg_label}                        
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_recording_path', default=f"/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_indicator_0and1/demos_{suffix}", type=str, help="where you want to record data")
    ...
    while (True): 
        ...
        if state == "move to goal":
            ...
            state = move_to_goal_state(sim_cache, plan_config, data_config, robot, plan_traj, eef_states, goal_indicators, balls_left, args)
