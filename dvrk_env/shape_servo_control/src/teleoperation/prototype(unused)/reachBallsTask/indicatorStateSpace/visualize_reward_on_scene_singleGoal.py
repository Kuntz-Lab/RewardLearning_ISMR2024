...
from reward import RewardNet1 as RewardNet
...
reward_model_path = '/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_indicator_singleGoal/weights/weights_paths24_noSlice'
reward_net.load_state_dict(torch.load(os.path.join(reward_model_path, "epoch 12")))
...
for _ in range(num_samples):
    eef_pose = np.random.uniform(low=[-0.07,-0.48,0.16+0.015], high=[0.07,-0.37,0.16+0.015+0.001], size=3)
    mid_pose = np.array([0.03, -0.44, 0.16+0.015]) #try swap y-coordinate of mid and goal
    goal_pose = np.array([-0.03, -0.44, 0.16+0.015])
    state = np.concatenate((eef_pose, mid_pose), axis=None)
