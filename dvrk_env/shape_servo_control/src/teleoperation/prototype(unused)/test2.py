...
if __name__ == "__main__":
    ...
    while (not close_viewer) and (not all_done): 
        ...
        #     cartesian_pose.position.x = -current_pose["pose"]["p"]["x"] + delta_pose.position.x
        #     cartesian_pose.position.y = -current_pose["pose"]["p"]["y"] + delta_pose.position.y
        #     cartesian_pose.position.z = current_pose["pose"]["p"]["z"] - ROBOT_Z_OFFSET + delta_pose.position.z
        ...
        if state == "move to goal":      
            ...
            if frame_count % 2 == 0:
                desired_pos = rospy.wait_for_message('/PSM1/measured_js', JointState).position
                
                pos_targets = robot.get_full_joint_positions()
                pos_targets[:1] = desired_pos[:1]
                pos_targets[3] = desired_pos[1]
                pos_targets[4:8] = desired_pos[2:]
                print("desired pos:", pos_targets)
