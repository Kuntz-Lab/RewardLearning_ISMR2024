...
model.load_state_dict(torch.load("/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_AE_boxCone_corrected/weights/weights_40000/epoch 150"))
...
for k in range(10):
    ...
    final_pc = []
    dataset_path = f"/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_AE_boxCone_corrected/processed_data_train"
    with open(os.path.join(dataset_path, "processed sample " + str(np.random.randint(0,10000)) + ".pickle"), 'rb') as handle:
    ...
    #final_pc.extend(sample_cone_box())
    # print(data["partial_pc"].shape)
    final_pc.extend(list(data["partial_pc"]))
