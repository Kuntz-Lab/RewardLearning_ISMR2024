...
def compute_predicted_reward(obj_emb_traj, eef_traj, reward_net, device): 
    obj_emb_traj = obj_emb_traj.unsqueeze(0).float().to(device)
    eef_traj = eef_traj.unsqueeze(0).float().to(device)
    reward = reward_net.cum_return(eef_traj, obj_emb_traj)[0]
...
if __name__ == "__main__":
    ...
    for i in range(num_data):
        ...
        eef_traj_1 = data["eef_traj_1"]
        eef_traj_2 = data["eef_traj_2"]
        label = data["label"]

        with torch.no_grad():
            rew_1 = compute_predicted_reward(emb_traj_1, eef_traj_1, reward_net, device)
            rew_2 = compute_predicted_reward(emb_traj_2, eef_traj_2, reward_net, device)
            # print(rew_1)
