...
def move_to_goal_state(sim_cache, plan_config, data_config, robot, plan_traj, args, record_pc=False):
    ...
    while traj_index <= len(plan_traj):
        ...
        if plan_config["frame"]%save_frame==0:
            ...
            if record_pc:
                pcd = get_partial_point_cloud(gym, sim, envs[0], cam_handles[0], cam_prop, color=[1,0,0], min_z = 0.009, visualization=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_recording_path', default=f"/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_push_corrected/demos_{suffix}", type=str, help="where you want to record data")
