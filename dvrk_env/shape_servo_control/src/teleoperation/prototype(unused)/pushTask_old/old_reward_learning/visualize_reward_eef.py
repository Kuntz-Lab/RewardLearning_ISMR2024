...
reward_model_path = '/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_push/weights_eef/weights_1'
reward_net.load_state_dict(torch.load(os.path.join(reward_model_path, "epoch 100")))

vis_data_path = "/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_push/demos_train/group 1 sample 4.pickle"
...
for sample in range(num_samples):
    eef_pose = np.array([0, -0.5, 0.05]) + np.random.uniform(low=[-0.4, -0.2, 0], high=[0.4, 0.4, 0], size=3)
...
plt.plot(cone_pose[0],cone_pose[1], "^", markersize=30)
