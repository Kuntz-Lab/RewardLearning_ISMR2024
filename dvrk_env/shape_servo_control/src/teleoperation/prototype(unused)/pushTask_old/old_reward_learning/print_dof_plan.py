...
def sample_balls_poses(num_balls, degree):
    ...
    weights_list, xy_curve_weights, balls_xyz = get_balls_xyz_curve(num_balls, degree=degree, radius=DIAMETER/2)
...
def set_balls_poses():
    rand_num_balls = 1 # random.randint(MIN_NUM_BALLS, MAX_NUM_BALLS)
...
def drop_ball(sim_cache, plan_config, eef_state):
    ...
    for i, ball_handle in enumerate(ball_handles):
        if is_overlap(eef_pose, all_balls_poses[i]):
...
def configure_isaacgym(gym, args):
    ...
    cam_positions = gymapi.Vec3(0.3, -0.8, 0.5)
...
def move_near_workspace(robot, dc_client):
    ...
    while(not success):
        ...
        cartesian_pose.position.y = -(-0.39)
    ...
    print("((((((((((((((((plan: ",plan_traj_with_gripper)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_recording_path', default=f"/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_trace_curve/demos_{suffix}_1ball", type=str, help="where you want to record data")
