...
def learn_reward(reward_network, optimizer, training_data_path, num_iter, l1_reg, checkpoint_dir):
    ...
    for epoch in range(num_iter):
        ...
        for i in range(num_data_pt):
            ...
            traj_i, traj_j = data["emb_traj_1"], data["emb_traj_2"]
            label = data["label"]
            # traj_i = torch.from_numpy(traj_i).float().to(device)
            # traj_j = torch.from_numpy(traj_j).float().to(device)
            traj_i = traj_i.float().to(device)
            traj_j= traj_j.float().to(device)
...
parser.add_argument('--rmp', default='/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_push/weights_no_eef/weights_100ex', type=str, help="path to reward model")
parser.add_argument('--tdp', default='/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_push/100_data_processed_train', type=str, help="path to training data")
