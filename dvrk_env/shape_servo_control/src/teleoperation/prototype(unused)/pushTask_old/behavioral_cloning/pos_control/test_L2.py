...
'''
Test trained L2 agent with cartesian (xyz) control
'''
...
def config_state(): 
    ...
    plan_config = {"cp_pose":gymapi.Transform(), "max_frame":180, "frame":0, \
...
def get_shape_servo_plan_state(sim_cache, plan_config, robot, dc_client, policy):
    state = "get shape servo plan"
    rospy.loginfo("**Current state: " + state) 
    ...
    print("=======================================================")
    print(f"#################### sim cache box pos: {box_pose}")
    print(f"#################### sim cache cone pos: {cone_pose}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', default=f"/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_push_BC_corrected/pos_control/weights/weights_L2", type=str, help="where you want to record data")
    ...
    model = BCPolicy(obs_dim=9, act_dim=3) 
    model.to(device)
    ...
    while (True): 
        ...
        if state == "get shape servo plan":
            ...
            state, plan_traj = get_shape_servo_plan_state(sim_cache, plan_config, robot, dc_client, model)
