...
def config_state(): 
    ...
    plan_config = {"cp_pose":gymapi.Transform(), "max_frame":300, "frame":0, \
...
def get_shape_servo_plan_state(sim_cache, plan_config, robot, dc_client, policy, AE_model):
    ...
    obj_emb = to_obj_emb(AE_model, device, pcd)
...
def move_to_goal_state(sim_cache, plan_config, data_config, robot, plan_traj, args, record_pc=False):
    ...
    while traj_index <= len(plan_traj):
        ...
        if plan_config["frame"]%save_frame==0:
            ...
            if record_pc:
                pcd = get_partial_point_cloud(gym, sim, envs[0], cam_handles[0], cam_prop, color=[1,0,0], min_z = 0.005, visualization=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_recording_path', default=f"/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_push_BC_corrected/demos_{suffix}", type=str, help="where you want to record data")
    parser.add_argument('--model_path', default=f"/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_push_BC_corrected/pointcloud_pos_control/weights/weights_max_prob_pc", type=str, help="policy")
    parser.add_argument('--AE_model_path', default="/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_AE_boxCone_corrected/weights/weights_40000/epoch 150", type=str, help="autoencoder")
    ...
    policy = ActorPC(act_dim=3, initial_std=1.0)
    policy.to(device)
    policy.load_state_dict(torch.load(os.path.join(model_path, "epoch 26")))
    policy.eval()
