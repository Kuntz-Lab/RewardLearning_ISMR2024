...
if __name__=="__main__":
    ...
    for i, num_data in enumerate(num_data_list):
        ...
        train_dataset_path = os.path.join(data_processed_root_path, f"processed_data_train_{max_num_data}")
        train_dataset = TrajDataset(train_dataset_path)
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=max_num_data//2, shuffle=True)
    ...
    # plot_bar_chart(xs=[2*j for j in range(len(num_data_list))], ys=train_accuracies, title="train accuracies on the largest dataset", path=plot_path, x_label="number of training data", y_label="accuracy", tick_labels=num_data_list)
    # plot_bar_chart(xs=[2*j for j in range(len(num_data_list))], ys=test_accuracies, title="test accuracies on the largest dataset", path=plot_path, x_label="number of training data", y_label="accuracy", tick_labels=num_data_list)
