...
model.load_state_dict(torch.load("/home/dvrk/LfD_data/ex_AE_cut/1ball/weights_pointconv/epoch_200"))
...
for k in range(10):
    ...
    final_pc = []
    dataset_path = f"/home/dvrk/LfD_data/ex_AE_cut/1ball/processed_data_test"
    with open(os.path.join(dataset_path, "processed sample " + str(np.random.randint(0,250)) + ".pickle"), 'rb') as handle:
    ...
    # z = 0.1
    # for i in range(1):
    #     final_pc.extend([[0,0,0]])
    #     #final_pc.extend(get_ball_pc(np.array([x, y, z]),256, 0.005))
    #     x+=0.01
    #     y-=0.01
    #     z+=0.0001
    ...
    open3d.visualization.draw_geometries([pcd, pcd2.translate((0,0,0.2))])  
