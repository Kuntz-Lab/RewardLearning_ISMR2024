...
'''
Collect random demos of cutting autonomously
'''
...
def config_state(sim_cache, data_config):
    ...
    balls_xyz = sim_cache["balls_xyz_init"]
    ...
    reach_cps = get_random_indices(num_reach, checkpoints_length)
    ##### expert demo #####
    # checkpoints = []
    ...
    for i in range(checkpoints_length):
        if i in reach_cps:
            ...
            #checkpoints.append(np.random.uniform(low=[-0.1, -0.45, EEF_GROUND_Z_OFFSET], high=[0.1,-0.35, 0.1], size=3))
            checkpoints.append(np.random.uniform(low=[-0.025, -0.4, EEF_GROUND_Z_OFFSET], high=[0.025,-0.375, 0.1], size=3))

    state = "get shape servo plan"

    plan_config = {"checkpoints": checkpoints, "current_cp":0, "reach_cps": reach_cps, "which_balls_reached": [0 for i in range(len(balls_xyz))], "last_ball_xyz":None, "num_balls_reached": 0, \
                  "eef_states":[], "pcds":[], "balls_xyzs_list": [], "last_ball_xyzs": [], "frame":0, "max_frame":250, "stay":False} #250
...
def move_to_goal_state(sim_cache, plan_config, data_config, robot, plan_traj, args, record_pc=True):
    ...
    while traj_index <= len(plan_traj):
        ...
        if plan_config["frame"]%save_frame==0:
            print("????? eef_states: ", np.array(list(eef_state["pose"]["p"])))
        ...
        if plan_config["frame"] < plan_config["max_frame"] - 1:
            ...
            if args.save_data:
                data = {"eef_states": plan_config["eef_states"], "pcds":plan_config["pcds"], "last_ball_xyzs": plan_config["last_ball_xyzs"], \
                    ...
                    "reach_cps": plan_config["reach_cps"], "which_balls_reached": plan_config["which_balls_reached"], "eef_ground_z_offset": EEF_GROUND_Z_OFFSET
                }
