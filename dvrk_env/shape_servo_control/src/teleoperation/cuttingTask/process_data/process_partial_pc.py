...
def process_and_save(partial_pc, data_processed_path, index):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_recording_path', type=str, help="where you recorded the data (pointcloud)")
    ...
    if not vis:
    # ############################### process and save ######################################
        num_groups = len(os.listdir(data_recording_path))
        print("num_groups to process: ", num_groups)
        
        processed_sample_idx = 0
        for group_idx in range(num_groups):
            with open(os.path.join(data_recording_path, f"group {group_idx}.pickle"), 'rb') as handle:
                data = pickle.load(handle)
            partial_pc = data["partial_pc"]
            process_and_save(partial_pc, data_processed_path, processed_sample_idx)
