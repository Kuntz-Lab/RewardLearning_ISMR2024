...
if __name__ == "__main__":
    ### CHANGE ####
    max_num_balls = 1
    root_folder = "/home/dvrk/RL_data/cut"
    reward_folder = "change_gt_ep10"
    data_recording_path = f"{root_folder}/{reward_folder}/data"
    num_episodes =351 #len(os.listdir(data_recording_path)) #641 #1280 the best episode for learned reward
    ...
    if plot_data:
        ...
        for episode_idx in range(step, num_episodes, step):

            if episode_idx % 50 == 0:
                print("========================================")
                print("current episode:", episode_idx, " , time passed:", timeit.default_timer() - start_time)
            
            data = get_episode_data(data_recording_path, episode_idx=episode_idx)
        ...
        print(avg_learned_rewards)
        episodes = [i for i in range(step, num_episodes, step)]
        plot_curve(xs=episodes, ys=avg_gt_rewards, title="average gt rewards of Cut", path=plot_path, x_label="episodes", y_label="avg reward")
        plot_curve(xs=episodes, ys=avg_learned_rewards, title="average learned rewards of Cut", path=plot_path, x_label="episodes", y_label="avg reward")
        plot_curve(xs=episodes, ys=avg_num_reaches, title="average num goal reached of Cut", path=plot_path, x_label="episodes", y_label="avg number of goal reached")
        plot_curve(xs=episodes, ys=percent_num_env_success, title=f"fraction of envs reaching {max_num_balls} goal in Cut", path=plot_path, x_label="episodes", y_label="fraction of env reaching the goal")
    ...
    if record_video:
        ...
        for episode_idx, im in enumerate(images_env_0):
            processed_image = Image.fromarray(im)
            processed_image.save(os.path.join(video_path, f"img_{episode_idx}.png"))
        ...
        plot_curve(xs=timesteps, ys=avg_balls_reached_per_step, title=f"average num goal reached per step in episode {last_episode_idx}", path=plot_path, x_label="timestep", y_label="avg number of goal reached")
        plot_curve(xs=timesteps, ys=env_0_balls_reached_per_step, title=f"env 0 num goal reached per step in episode {last_episode_idx}", path=plot_path, x_label="timestep", y_label="number of goal reached")

        ######### make video ##########
        print("making video from images...")
        image_filenames = [f"img_{i}.png" for i in range(len(images_env_0))]
        make_video(video_path, image_filenames, save_path=f'{root_folder}/{reward_folder}/rl_video_{last_episode_idx}.mp4' , fps=3)
