...
class GraspClient:
    def __init__(self):
        ...
        self.save_visual_data_pre_path = rospy.get_param('~save_visual_data_pre_path', '/home/dvrk/dvrk_grasp_data/visual/')
        ...
        self.data_recording_path = rospy.get_param('~data_recording_path', '/home/dvrk/dvrk_grasp_data/')
    ...
    def add_obstacle_client(self, remove_obs, x=0, y=0, z=0, rot_x=1e-6, rot_y=1e-6, rot_z=1e-6, rot_w=1.000000):
        rospy.loginfo('Waiting for service add_obstacle_server.')
        rospy.wait_for_service('add_obstacle_server')
        rospy.loginfo('Calling service add_obstacle_server.')
        try:
            proxy = rospy.ServiceProxy('add_obstacle_server', AddObstacle)
            request = AddObstacleRequest()
            request.remove_obs = remove_obs       
            request.x = x
            request.y = y
            request.z = z
            request.rot_x = rot_x
            ...
            request.rot_z = rot_z
            request.rot_w = rot_w
            # request.obs_name = String()
            # request.obs_mesh_path = String()
            # request.obs_name.data = self.to_bytes(obs_name.encode('utf-8'))
            # request.obs_mesh_path.data = self.to_bytes(obs_mesh_path.encode('utf-8'))
            self.response = proxy(request) 
        except (rospy.ServiceException):
            rospy.loginfo('add_obstacle_server call failed')
        rospy.loginfo('add_obstacle_server is executed %s.'
                %str(self.response.success))
        
        if not self.response.success:
            rospy.loginfo('error adding or removing obstacle')
    ...
    def gen_object_pose(self):
        ...
        z_orientation = np.random.uniform(-1/4 * np.pi, 1/4 * np.pi)
