...
import rospy
import sys
sys.path.append('/home/dvrk/shape_servo_DNN')
...
def get_partial_point_cloud(gym, sim, env, cam_handle, cam_prop, color=None, min_z = 0.005, visualization=False):
...
def down_sampling(pc, num_pts=1024):
    ...
    #print(f"indices shape: {farthest_indices.shape}")
    pc = pc[farthest_indices.squeeze()]  
    return pc

# def down_sampling_batch(pc, num_pts=1024):
#     farthest_indices,_ = farthest_point_sampling(pc, num_pts)
#     print(f"indices shape: {farthest_indices.shape}")
#     pc = pc[np.arange(0, pc.shape[0]).reshape(-1, 1), farthest_indices, :]  
#     return pc
