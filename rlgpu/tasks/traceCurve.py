...
from traceSurface.behavioral_cloning.pointcloud_pos_control.policy_BC_PC import ActorPC
...
def sample_balls_poses(num_balls):
    ...
    # #rand_offset = 0.02
    # weights_list, xy_curve_weights, balls_xyz = get_balls_xyz_curve(num_balls, degree=degree, offset=rand_offset, verbose=False)
    weights_list, xy_curve_weights, balls_xyz = get_balls_xyz_curve(num_balls, curve_type=CURVE_TYPE, verbose=False)
...
def gt_reward_function(all_envs_eef_pose, all_envs_balls_xyz, remain_mask, last_ball_xyz, overlap, use_bonus=True):
    ...
    ########## gaussian
    # var=0.2
    # rewards = 1/(var*(2*np.pi)**0.5)*np.exp(-rewards/(2*var**2))
    # # handle when all balls were touched
    # remain_counts = np.sum(remain_mask, axis=1)
    # reward_copy = np.copy(rewards)
    # no_ball_reward =1/(var*(2*np.pi)**0.5)*np.exp(-np.sum((all_envs_eef_pose - last_ball_xyz)**2, axis=1)/(2*var**2)) #(num_envs, )
    # rewards = np.where(remain_counts==0, no_ball_reward, reward_copy)
    ...
    # remain_counts = np.sum(remain_mask, axis=1)
    # reward_copy = np.copy(rewards)
    # no_ball_reward = -np.sum((all_envs_eef_pose - last_ball_xyz)**2, axis=1) #(num_envs, )
    # rewards = np.where(remain_counts==0, no_ball_reward, reward_copy)
...
class TraceCurve(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.rl_data_idx = 0
        self.save_idx = 10
        self.rl_data_path = "/home/dvrk/RL_data/traceCurve/deletethis/data" #gt_reward_eef_pos_2ball
        ...
        # self.BC_policy.load_state_dict(torch.load("/home/dvrk/LfD_data/ex_trace_curve_corrected/BC/weights_BC_perfect/epoch_200"))
        # self.BC_policy.eval()
        ...
        self.tvc_behavior = TaskVelocityControlMultiRobot(list(dummy_desired_positions), self.robots, self.num_envs, dt, 3, vel_limits=self.vel_limits, error_threshold = 5e-3) #5e-3 5e-6             
    ...
    def compute_observations(self, env_ids=None):
        ...
        # processed_pcs = []
        # max_num_points = 300
        # pc_is_empty = torch.zeros((self.num_envs, )).to(self.device)
        ...
        #     pc = get_partial_pointcloud_vectorized_seg_ball(self.gym, self.sim, self.envs[i], self.cam_handles[i], vinv, proj, self.cam_prop, self.dropped_mask[i], self.ball_seg_Ids[i], color=None, min_z = 0.01, visualization=False, device="cuda")
        ...
        # pc_is_empty = pc_is_empty.unsqueeze(1).expand(-1, 256)

        # ################# replace emb of empty pc with emb of previous pc
        # self.obj_embs = torch.where(pc_is_empty==1, self.obj_embs, emb)
    ...
    def pre_physics_step(self, actions):
        ...
        if self.use_dof_vel_action:
            ...
            ##### BC
            # actions_clipped = self.BC_policy(self.obs_buf)
            # actions_clipped[:, 0] = torch.clamp(actions_clipped[:,0], -0.1, 0.1)
            # actions_clipped[:, 1] = torch.clamp(actions_clipped[:,1], -0.6, -0.4)
            # actions_clipped[:, 2] = torch.clamp(actions_clipped[:,2], 0.03, 0.1)
...
# #from util.isaac_utils import farthest_point_sample_batched
# sys.path.append("/home/dvrk/catkin_ws/src/dvrk_env/shape_servo_control/src/teleoperation/traceSurface")
# sys.path.append("/home/dvrk/catkin_ws/src/dvrk_env/shape_servo_control/src/teleoperation")
# sys.path.append("/home/dvrk/catkin_ws/src/dvrk_env/shape_servo_control/src/teleoperation/traceSurface/pointcloud_representation_learning")
# from curve import *
# from compute_partial_pc import get_partial_pointcloud_vectorized_seg_ball, farthest_point_sample_batched, get_all_bin_seq_driver
# from traceSurface.reward import RewardNetPointCloudEEF
# from traceSurface.pointcloud_representation_learning.architecture import AutoEncoder
# import random
# import time
...
# def set_balls_poses(balls_config_data_path, episode):
#     all_remain_bin_seq = tuple(np.array([1 for i in range(MAX_NUM_BALLS)])==1)
#     with open(os.path.join(balls_config_data_path, f"group {episode} sample {all_remain_bin_seq}.pickle"), 'rb') as handle:
#         data = pickle.load(handle)
#     all_balls_xyz = data["balls_xyz"]
#     all_balls_poses = []
#     for xyz in all_balls_xyz:
#         ball_pose = gymapi.Transform()
#         pose = [xyz[0], xyz[1], xyz[2]]
#         ball_pose.p = gymapi.Vec3(*pose)
#         all_balls_poses.append(ball_pose)
        
#     return all_balls_xyz, all_balls_poses
...
#         ############# important for loading pre-generated balls partial pointclouds and poses
#         self.balls_config_data_path = f"/home/dvrk/RL_data/traceCurve/preprocessed_{MAX_NUM_BALLS}ball_data"
...
#         self.encoder =  AutoEncoder(num_points=256, embedding_size=256).to(self.device)
#         self.encoder.load_state_dict(torch.load("/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_AE_balls_corrected/weights_straight3D_partial_flat_2ball_varied/weights_1/epoch 150"))

#         self.reward_net = RewardNetPointCloudEEF()
#         self.reward_net.to(self.device)
#         reward_model_path = '/home/dvrk/shape_servo_data/teleoperation/sanity_check_examples/ex_trace_curve_corrected/weights_straight_3D_flat_2ball_varied/weights_1'
#         self.reward_net.load_state_dict(torch.load(os.path.join(reward_model_path, "epoch 200")))
#         self.reward_net.eval()
...
#         self.rl_data_idx = 0
#         self.save_idx = 10
#         self.rl_data_path = "/home/dvrk/RL_data/traceCurve/gt_reward_0/data"
#         os.makedirs(self.rl_data_path, exist_ok=True)
...
#         envs_all_balls_poses = []
#         self.all_envs_balls_xyz = []
#         all_balls_xyz, all_balls_poses = set_balls_poses(self.balls_config_data_path, episode=0)
#         for i in range(self.num_envs):
#             envs_all_balls_poses.append(all_balls_poses)
#             self.all_envs_balls_xyz.append(all_balls_xyz)
...
#     def compute_reward(self, actions):
#         ######################### gt reward ########################
#         gt_rewards = gt_reward_function(self.states["eef_pos"].cpu().numpy(), self.all_envs_balls_xyz, self.remain_mask, self.last_ball_xyz, self.overlap, use_bonus=True)
#         print("gt reward: ", np.around(gt_rewards[0], decimals=4))
...
#         ############## use pre-processed object embedding ############
#         empty_tuple = tuple([0 for i in range(MAX_NUM_BALLS)])
#         for i in env_ids:
#             if tuple(self.remain_mask[i]) == empty_tuple:
#                 continue
#             self.obj_embs[i] = self.emb_dict[tuple(self.remain_mask[i])][i]
...
#         # pc_is_empty = torch.zeros((self.num_envs, )).to(self.device)
#         # for i in env_ids:
#         #     #pc = get_partial_pointcloud_vectorized_seg_ball_from_buffer(self.depth_buffers[i], self.seg_buffers[i], self.V_inv, self.P, self.cam_width, self.cam_height, self.dropped_mask[i], self.ball_seg_Ids[i], color=None, min_z=0.01, visualization=False, device="cpu")
#         #     pc = get_partial_pointcloud_vectorized_seg_ball(self.gym, self.sim, self.envs[i], self.cam_handles[i], self.cam_prop, self.dropped_mask[i], self.ball_seg_Ids[i], color=None, min_z = 0.01, visualization=False, device="cpu")
#         #     processed_pc = np.zeros((max_num_points, 3))
#         #     if len(pc)==0:
#         #         pc_is_empty[i] = 1
#         #     else:
...
#        ############################# reset balls ######################################
#         # all_balls_xyz, _ = set_balls_poses(self.balls_config_data_path, episode=self.rl_data_idx)
#         # for i in env_ids:
#         #     self.all_envs_balls_xyz[i] = np.array(all_balls_xyz)
    
#         root_state = torch.clone(self._root_state).cpu().numpy()
#         balls_states = root_state[:, 1:1+MAX_NUM_BALLS, :]

#         # Write these new init balls pos to the sim states
#         new_pos = self.all_envs_balls_xyz[env_ids, :, :]
#         balls_states[env_ids, :, :3]=new_pos
#         # no rotation
#         balls_states[env_ids, :, 3:7]=torch.tensor([0.0, 0.0, 0.0, 1.0], device=self.device).repeat(num_reset, MAX_NUM_BALLS, 1).cpu().numpy()
#         # 0 velocity
...
#             for i in range(self.num_envs):
#                 #pc = get_partial_pointcloud_vectorized_seg_ball(self.gym, self.sim, self.envs[i], self.cam_handles[i], self.cam_prop, np.array(bin_seq), self.ball_seg_Ids[i], color=None, min_z = 0.01, visualization=False, device="cpu")
#                 with open(os.path.join(self.balls_config_data_path, f"group {0} sample {tuple(np.array(bin_seq)==1)}.pickle"), 'rb') as handle:
#                     data = pickle.load(handle)
#                 pc = data["partial_pc"]
#                 print(f"pc shape: env {i}, {pc.shape}")
#                 processed_pc = np.zeros((max_num_points, 3))
#                 pad_point = pc[-1, :]
#                 processed_pc[:len(pc), :] = pc
#                 processed_pc[len(pc):, :] = pad_point
...
#         ######################### drop balls when overlap with eef #############################
#         eef_ball_diff = np.sum((all_envs_eef_pose - remain_balls_xyz)**2, axis=2) # (num_envs, max_num_balls)
#         overlap = eef_ball_diff <= 0.0003 #0.00025 #0.0001
